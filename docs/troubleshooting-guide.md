...
## `terraform plan` 显示 EKS 的 `security_group_ids` 配置需要调整

* **问题现象 (What Happened)**：执行 `terraform plan` 时，Terraform 检测到 EKS 集群的 `vpc_config.security_group_ids` 与当前状态不符，计划移除已存在的安全组 ID（如 `sg-0e93d691d659c1eda`）。或者当您手动指定安全组 ID 后，Terraform 计划中显示要删除 AWS 自动创建的安全组。

* **背景场景 (Context)**：
  - EKS 集群在创建时，如果未在 `vpc_config` 中指定 `security_group_ids`，AWS 会自动创建一个默认的**集群安全组（Cluster Security Group）**。
  - 当您通过非 Terraform 方式（如 eksctl）创建集群后导入状态时，该默认安全组已存在。
  - 在 Terraform 配置中显式指定 `security_group_ids` 后，Terraform 期望只保留这些安全组。

* **复现方式 (How to Reproduce)**：
  1. 使用 eksctl 创建集群（自动生成安全组 SG-A）
  2. 导入集群到 Terraform 状态
  3. 在 `aws_eks_cluster` 资源中添加：
     ```hcl
     vpc_config {
       security_group_ids = ["sg-custom"] # 您的自定义安全组
       subnet_ids         = var.subnets
     }
     ```
  4. 运行 `terraform plan` 将显示：
     ```
     ~ security_group_ids = [SG-A] -> ["sg-custom"]
     ```

* **根因分析 (Root Cause)**：
  - **错误理解**：认为 EKS 总会自动创建额外安全组
  - **正确行为**：当显式指定 `security_group_ids` 时：
    - ✅ EKS **不会创建**新的安全组
    - ✅ 仅使用您提供的安全组
    - ❌ Terraform 会移除状态中存在的、但配置中未声明的安全组
...
* **修复方法 (Fix / Resolution)**：
  1. **完全控制模式（推荐）**：
     ...
     resource "aws_security_group_rule" "eks_api" {
       security_group_id = "sg-0e93d691d659c1eda"
       type              = "ingress"
       from_port         = 443
       to_port           = 443
       protocol          = "tcp"
       cidr_blocks       = ["10.0.0.0/16"] # 使用您的 VPC CIDR
     }
     ```
  ...
  2. **混合模式（保留自动创建的安全组）**：
     ...
     vpc_config {
       security_group_ids = concat(
         ["sg-0e93d691d659c1eda"], # 您的自定义安全组
         [aws_eks_cluster.this.vpc_config[0].cluster_security_group_id] # 自动创建的SG
       )
       subnet_ids = var.subnets
     }
     ```
...
* **相关命令 (Commands Used)**：
  - 查看当前安全组：
    ```bash
    aws eks describe-cluster --name dev --query "cluster.resourcesVpcConfig.securityGroupIds"
    ```
  - 检查安全组规则：
    ```bash
    aws ec2 describe-security-group-rules \
      --filter Name="group-id",Values="sg-0e93d691d659c1eda"
    ```
  - 安全组规则验证（最小要求）：
    ```bash
    # 入站规则
    aws ec2 describe-security-groups \
      --group-ids sg-0e93d691d659c1eda \
...
* **关键注意事项**：
  - 每日销毁重建时，使用独立安全组：
    ```hcl
    resource "aws_security_group" "persistent_sg" {
      name_prefix = "eks-core-sg-"
      vpc_id      = var.vpc_id
      lifecycle {
  ...
  - 规则要求：
    | 方向 | 端口 | 协议 | 源/目标 | 用途 |
    |---|---|---|---|---|
    | 入站 | 443 | TCP | 节点安全组 | 节点到 API 服务器 |
    | 出站 | 全部 | 全部 | 0.0.0.0/0 | 互联网访问 |

* **调试技巧**：
  若通信失败，检查控制平面日志：
  ```bash
  aws logs filter-log-events \
    --log-group-name "/aws/eks/dev/cluster" \
