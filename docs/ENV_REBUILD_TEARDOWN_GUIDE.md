...
混沌工程组件 `Chaos Mesh` 在开启时会部署到 `chaos-testing` 命名空间，仅包含 controller 与 daemonset；示例实验清单位于 `deploy/k8s/experiments/chaos/`。
...
- [x] **IRSA S3 权限冒烟（aws-cli Job）**
  - 运行临时 Job 验证 STS 与 S3 前缀权限：
    ```bash
    kubectl apply -f deploy/k8s/jobs/smoke/awscli-smoke.yaml
...
2. **选择实验清单**：仓库提供的实验 YAML 位于 `deploy/k8s/experiments/chaos/`，示例包括：
   - `experiment-net-latency.yaml`：向 `task-api` 注入 200ms 网络延迟；
   - `experiment-pod-kill.yaml`：随机终止 `task-api` 的一个 Pod。
3. **应用实验**：
   ```bash
   kubectl apply -f deploy/k8s/experiments/chaos/experiment-net-latency.yaml
   # 或
   kubectl apply -f deploy/k8s/experiments/chaos/experiment-pod-kill.yaml
   ```
4. **观察效果**：
   ```bash
...
5. **清理实验**：实验完成后删除对应 YAML，恢复正常状态：
   ```bash
   kubectl delete -f deploy/k8s/experiments/chaos/experiment-net-latency.yaml
   # 或
   kubectl delete -f deploy/k8s/experiments/chaos/experiment-pod-kill.yaml
   ```
