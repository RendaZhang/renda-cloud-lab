...
* `make start` — **启动基础设施资源**：执行 Terraform 将 NAT 网关、ALB 等高成本资源启用（根据需要也会确保 EKS 集群处于运行状态）。通常在每天实验开始时运行，恢复网络出口和对外服务能力。
* `make stop` — **停止基础设施资源**：执行 Terraform 关闭 NAT 网关、ALB 等非必要资源（将 `create_nat`、`create_alb` 等标志置为 false），保留基础设施状态但暂停对外网络访问。适用于每日实验结束时销毁高成本资源，降低支出。
